// ESP32-C6 version of esp32-S3-12-04-25.ino
// - BLE stack migrated to NimBLE (compatible with ESP32-C6)
// - LED logic, animations, pins kept identical to S3 sketch
...
void shootingStarAnimation(int red, int green, int blue, int tail_length, int delay_duration, int interval, int direction) {
    static unsigned long previousMillisSS = 0;
    static int countSS = 0;

    unsigned long currentMillisSS = millis();
    if (currentMillisSS - previousMillisSS >= (unsigned long)interval) {
        previousMillisSS = currentMillisSS; 
        countSS = 0;  // Reset animasi
    }
    ...
    if (direction == -1) {
        leds[NUM_LEDS - countSS] = CRGB(red, green, blue);
    } else {
        leds[countSS] = CRGB(red, green, blue);
    }
    ...
    FastLED.show();
    countSS++;  // Maju ke LED berikutnya
...
class CharacteristicCallback : public NimBLECharacteristicCallbacks {
  void onWrite(NimBLECharacteristic* pCharacteristic) override {
    ...
    const uint8_t* received_data = (const uint8_t*)val.data();
    int length = (int)val.length(); // Ambil panjang data
    ...
    if (length >= 2 && received_data[0] == 2) {
      led_status = received_data[1];
    //Pemilihan Animasi
    } else if (length >= 2 && received_data[0] == 9) {
      blinking = false;
    ...
    //Color Picker
    } else if (length >= 4 && received_data[0] == 37) {
      led_red = received_data[1];
      led_green = received_data[2];
      led_blue = received_data[3];
    } else if (length >= 2 && received_data[0] == 51) { // **Blinking Speed**
    ...
    } else if (length >= 2 && received_data[0] == 32) {
      led_brightness = received_data[1];
    //??
    } else if (length >= 2 && received_data[0] == 48) {
      if (received_data[1] == 1) {
        sendInfo();
      }
    } else if (length >= 6 && received_data[0] == 80) { // Cek Start Byte
...
void sendInfo() {
  ...
  // If you want to push updates, uncomment the next line:
...
void loop() {
  if (deviceConnected) { // Jika perangkat BLE terhubung
    ...
    if (led_status) { 
      ...
      if (blinking && currentMillis - previousMillis >= (unsigned long)blinkSpeed) { 
